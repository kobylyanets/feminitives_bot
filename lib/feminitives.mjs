...
function construct_feminitive(stem, ending, gap) {
  return gap ? stem + ' \u26A7 ' + ending : stem + "_" + ending;
}
...
//Создание феминитива
const make_feminitives = function make_feminitives(word) {
...
const get_wiktionary = async (term, ctx) => {
  var cors_url = "https://coors.now.sh/";
  var wiki_url = "https://ru.wiktionary.org/w/index.php?action=raw&title=" + term;
  ...
  const res = await fetch(encodeURI(wiki_url), {

    method: 'GET'
  });
  const text = await res.text();
  return parseWikiPage(text);
};
